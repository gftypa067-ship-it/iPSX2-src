...
// ================================================================
// ✅ إضافة دالة مساعدة للحصول على الجهاز الافتراضي (متوافقة مع iOS 16)
// ================================================================
static id<MTLDevice> getDefaultMetalDevice(void)
{
    // iOS 16+ تدعم MTLCreateSystemDefaultDevice مباشرة
    id<MTLDevice> device = MTLCreateSystemDefaultDevice();
    if (device) {
        Console.WriteLn("Metal: Using default system device: %s", [[device name] UTF8String]);
        return device;
    }
    
    // في حال فشل (احتياطي) - لكن هذا لا يحدث في iOS 16+
    Console.Error("Metal: Failed to get default device!");
    return nil;
}
...
static MRCOwned<id<MTLLibrary>> loadMainLibrary(id<MTLDevice> dev)
{
    ...
    // ================================================================
    // ✅ استخدام MTLCreateSystemDefaultDevice إذا كان متوفراً
    // ================================================================
    id<MTLDevice> defaultDevice = getDefaultMetalDevice();
    if (defaultDevice) {
        return MRCTransfer([defaultDevice newDefaultLibrary]);
    }
...
GSMTLDevice::GSMTLDevice(MRCOwned<id<MTLDevice>> dev)
{
    ...
    // ================================================================
    // ✅ إذا كان الجهاز الذي تم تمريره غير صحيح، استخدم الجهاز الافتراضي
    // ================================================================
    if (!dev) {
        dev = MRCTransfer(getDefaultMetalDevice());
        if (!dev) {
            Console.Error("Metal: Failed to obtain a valid MTLDevice!");
            return;
        }
    }
