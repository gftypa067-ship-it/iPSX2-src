...
void GSDeviceMTL::ResizeWindow(s32 new_window_width, s32 new_window_height, float new_window_scale)
{
	...
	// Always update surface_width/height even when m_layer is null so that
	// AttachSurfaceOnMainThread (which runs after AcquireRenderWindow) reads the
	// correct size when it calls [m_layer setDrawableSize:].
